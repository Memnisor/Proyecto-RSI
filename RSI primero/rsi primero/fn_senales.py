...
def detectar_senales(rsi: pd.Series,
    ...
    # SELL: ayer estaba EN sobrecompra, hoy SALIÓ hacia abajo
    mask_sell = (
        rsi_ayer.notna() &
        rsi.notna()      &
        (rsi_ayer >= sobrecompra) &  # ayer: dentro de la zona de sobrecompra
        (rsi < sobrecompra)          # hoy:  salió de esa zona → cruce ↓
    )
